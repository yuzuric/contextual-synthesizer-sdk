...
`contextual-synthesizer-sdk` is built on top of [Xiaomi MiMo](https://platform.xiaomimimo.com), the open-source large language model series from Xiaomi. The project demonstrates how to integrate MiMo into production systems using its OpenAI-compatible API.
...
```python
import asyncio
from contextual_synthesizer_sdk.client import MiMoClient
...
```
┌─────────────┐     ┌──────────────┐     ┌─────────────────┐
│  Application │───▶│  contextual_syn │───▶│  Xiaomi MiMo    │
│              │     │  (this repo) │     │  (LLM API)      │
└─────────────┘     └──────────────┘     └─────────────────┘
                            │
                            ▼
...
├── config.yaml
├── main.py
├── src/contextual_synthesizer_sdk/
│   ├── retriever.py
│   ├── client.py
│   └── config.py
├── tests/
